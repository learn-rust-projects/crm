...
#[derive(Debug, Clone, Dummy, Serialize, Deserialize, PartialEq, Eq, Hash)]
enum Gender {
    Female,
    Male,
    Unknown,
...
async fn main() -> Result<()> {
    println!("{:?}", [1, 2, 3].to_vec());
    let user = Faker.fake::<UserStat>();
    println!("{}", len(&user));
    let users: Vec<_> = Vec::from([user]);
    println!("{:?}", list_to_string(&users[0].recent_watched));
...
async fn raw_insert(users: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
    let mut sql = String::with_capacity(10 * 1000 * 2000);
    sql.push_str("
...
fn list_to_string(list: &[i32]) -> String {
    format!("ARRAY{:?}", list)
}
...
impl Dummy<UniqueEmail> for String {
    fn dummy_with_rng<R: RngExt + ?Sized>(_: &UniqueEmail, rng: &mut R) -> String {
