...
impl UserStatsService {
    ...
    pub async fn raw_query(&self, req: RawQueryRequest) -> ServiceResult<ResponseStream> {
        // TODO: query must only return email and name, so we should use sqlparser to
        // parse the query
        let Ok(ret) = sqlx::query_as(&req.query).fetch_all(&self.inner.pool).await else {
...
mod tests {
    ...
    use crate::{
        AppConfig,
        pb::user_stats::{IdQuery, QueryRequestBuilder, TimeQuery},
    };
    ...
    async fn query_should_work() -> Result<()> {
        ...
        let query = QueryRequestBuilder::default()
            ...
            .timestamp(("last_visited_at".to_string(), tq(Some(360), None)))
            .id(("viewed_but_not_started".to_string(), id(&[293748])))
            .build()
            .unwrap();
